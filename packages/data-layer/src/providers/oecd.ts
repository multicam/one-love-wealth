...
export class OECDProvider extends BaseProvider<OECDConfig> {
  override readonly name = 'OECD';
  override readonly cachePrefix = 'OECD';
  protected override defaultTTL = 7 * 24 * 60 * 60 * 1000; // 7 days

  protected override buildRequestConfig(config: OECDConfig): RequestConfig {
    ...
    if (config.startTime) params.start = config.startTime;
    if (config.endTime) params.end = config.endTime;
  ...
  protected override getCacheKeyComponents(config: OECDConfig): CacheKeyComponents {
  ...
  protected override transformResponse(json: unknown, config: OECDConfig): DataPoint[] {
    const response = json as OECDResponse;

    if (!response.dataSets || !response.structure) {
      throw new DataLayerError(
        'Invalid OECD response format',
    ...
    if (!dataset || !dataset.observations) {
      throw new DataLayerError(
        `No data found for ${config.location}/${config.indicator}`,
        ErrorCode.NOT_FOUND,
        this.name
      );
    }
    ...
    if (!timeDimension || !timeDimension.values) {
      throw new DataLayerError(
        'No time dimension found in OECD response',
        ErrorCode.INVALID_RESPONSE,
        this.name
      );
    }
  ...
  private convertPeriodToDate(period: string): string {
  ...
  protected override generateMockData(config: OECDConfig): DataPoint[] {
    const points: DataPoint[] = [];
