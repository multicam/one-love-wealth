...
export class CoinGeckoProvider extends BaseProvider {
    ...
    buildRequestConfig(config) {
        ...
        if (endpoint === 'market_chart') {
            ...
            if (config.precision !== undefined)
                params.precision = String(config.precision);
        }
        if (endpoint === 'ohlc') {
    ...
    transformResponse(json, config) {
        const endpoint = config.endpoint || 'market_chart';
        if (endpoint === 'market_chart') {
            return this.transformMarketChartResponse(json);
        }
        if (endpoint === 'ohlc') {
            return this.transformOHLCResponse(json);
        }
        if (endpoint === 'simple_price') {
            return this.transformSimplePriceResponse(json, config);
        }
        throw new DataLayerError(`Unknown endpoint: ${endpoint}`, ErrorCode.INVALID_RESPONSE, this.name);
    }
    transformMarketChartResponse(response) {
        if (!response.prices) {
            throw new DataLayerError('Invalid market_chart response: missing prices', ErrorCode.INVALID_RESPONSE, this.name);
        }
    ...
    transformOHLCResponse(response) {
        if (!Array.isArray(response)) {
            throw new DataLayerError('Invalid OHLC response: expected array', ErrorCode.INVALID_RESPONSE, this.name);
        }
    ...
    transformSimplePriceResponse(response, config) {
        const coinData = response[config.coinId];
        if (!coinData) {
            throw new DataLayerError(`Invalid simple_price response: missing data for ${config.coinId}`, ErrorCode.INVALID_RESPONSE, this.name);
        }
