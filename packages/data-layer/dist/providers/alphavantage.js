...
export class AlphaVantageProvider extends BaseProvider {
    name = 'Alpha Vantage';
    cachePrefix = 'ALPHAVANTAGE';
    defaultTTL = 24 * 60 * 60 * 1000; // 24 hours (due to 25 req/day limit)
    buildRequestConfig(config) {
        ...
        if (config.fromCurrency !== undefined)
            params.from_currency = config.fromCurrency;
        if (config.toCurrency !== undefined)
            params.to_currency = config.toCurrency;
        if (config.dateRange?.start !== undefined)
            params.start = config.dateRange.start;
        if (config.dateRange?.end !== undefined)
    ...
    getCacheKeyComponents(config) {
        ...
        if (config.fromCurrency)
            params.fromCurrency = config.fromCurrency;
        if (config.toCurrency)
            params.toCurrency = config.toCurrency;
        return {
    ...
    transformResponse(json, config) {
        ...
        if (response['Error Message']) {
            throw new DataLayerError(String(response['Error Message']), ErrorCode.INVALID_RESPONSE, this.name);
        }
        if (response['Note']) {
            throw new DataLayerError('Alpha Vantage API rate limit exceeded', ErrorCode.RATE_LIMITED, this.name);
        }
        const timeSeriesKey = TIME_SERIES_KEYS[config.function] || 'Time Series (Daily)';
        const timeSeries = response[timeSeriesKey];
        if (!timeSeries) {
            throw new DataLayerError(`No data found for ${config.symbol}`, ErrorCode.NOT_FOUND, this.name);
        }
        const points = [];
