<script lang="ts">
	...
	const numericFields = $derived.by(() => {
		const currentStrategy =
			strategy.selectedStrategy as StrategyDefinition | null;
	...
	<div class="space-y-4">
		<div class="flex items-center justify-between">
			...
			<span class="text-xs text-text-secondary">
				{numericFields.length} parameter{numericFields.length !== 1
					? "s"
					: ""}
			</span>
		</div>
					...
					{#if range}
						<div class="grid grid-cols-3 gap-2">
							...
							<div>
								<label
									for="{field.key}-min"
									class="text-xs text-text-secondary mb-1 block"
								>
									Min
								</label>
								<input
						...
						{#if !valid}
							<div class="mt-2 text-xs text-red-500">
								Invalid range: min must be less than max, and
								step must be positive and smaller than range
							</div>
						{/if}
...
{:else}
	<div class="p-4 rounded-lg bg-surface/50 border border-border text-center">
		<p class="text-sm text-text-secondary">
			No numeric parameters to optimize. Selected strategy only has fixed
			parameters.
		</p>
	</div>
{/if}
