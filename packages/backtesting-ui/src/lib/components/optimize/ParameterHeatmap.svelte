<script lang="ts">
	...
	function renderHeatmap() {
		...
		const xValues = Array.from(new Set(results.map((r) => r.params[xParam]))).sort(
			(a, b) => a - b
		);
		const yValues = Array.from(new Set(results.map((r) => r.params[yParam]))).sort(
			(a, b) => a - b
		);

		// Create scales
		const xScale = d3.scaleBand().domain(xValues.map(String)).range([0, width]).padding(0.05);

		const yScale = d3.scaleBand().domain(yValues.map(String)).range([height, 0]).padding(0.05);

		// Color scale
		const objectiveValues = results.map((r) => r.objectiveValue);
		const colorScale = d3
			.scaleSequential()
			.domain([Math.min(...objectiveValues), Math.max(...objectiveValues)])
		...
		svg
			...
			.call(d3.axisBottom(xScale).tickValues(xValues.filter((_, i) => i % 2 === 0).map(String)))
			.selectAll('text')
			.attr('fill', '#9ca3af')
		...
		svg
			.append('g')
			.call(d3.axisLeft(yScale).tickValues(yValues.filter((_, i) => i % 2 === 0).map(String)))
			.selectAll('text')
			.attr('fill', '#9ca3af')
			.style('font-size', '10px');
...
{#if optimization.hasResult && availableParams().length >= 2}
	<div class="space-y-4">
		...
		<div class="grid grid-cols-2 gap-3">
			<div>
				<label for="x-param" class="text-xs font-medium text-text-secondary mb-1 block">
					X Axis
				</label>
				<select
					id="x-param"
			...
			<div>
				<label for="y-param" class="text-xs font-medium text-text-secondary mb-1 block">
					Y Axis
				</label>
				<select
					id="y-param"
		...
		<!-- Heatmap Container -->
		<div bind:this={container} class="bg-surface rounded-lg border border-border p-4"></div>

		<!-- Help Text -->
		<div class="text-xs text-text-secondary">
			<strong>How to read:</strong> Each cell represents a parameter combination. Brighter colors
...
{:else}
	<div class="p-4 rounded-lg bg-surface/50 border border-border text-center">
		<p class="text-sm text-text-secondary">Run an optimization to visualize parameter space</p>
	</div>
{/if}
